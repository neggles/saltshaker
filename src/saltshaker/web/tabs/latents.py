...
def make_latents(model_name_or_path, config_path, buckets_path, progress=gr.Progress()):
    try:
        ...
        encode(model_name_or_path, buckets_path, config, progress=progress)
...
def load():
    model_name_or_path = gr.Textbox(lines=1, label="Model Path", value="model", interactive=True)
    ...
    latents_button.click(
        fn=make_latents, inputs=[model_name_or_path, config_load_path, bucket_save_path], outputs=[log_output]
    )
