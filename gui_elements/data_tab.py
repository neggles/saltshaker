...
def load():
    ...
    max_area = gr.Slider(value=768, minimum=0, maximum=2048, label="Max Area (input will be squared)",
                         interactive=True, step=64)
    with gr.Accordion(label="Bucket settings (advanced)", open=False):
        bucket_count = gr.Slider(value=32, minimum=0, maximum=128, label="Bucket Count", interactive=True, step=1)
        bucket_min = gr.Slider(value=256, minimum=0, maximum=512, label="Bucket Edge Min", interactive=True, step=64)
        bucket_max = gr.Slider(value=1536, minimum=0, maximum=2048, label="Bucket Edge Max", interactive=True, step=64)
    config_save_path = gr.Textbox(lines=1, label="Config Save Path", value="train_config.json", interactive=True)
    bucket_button = gr.Button(label="Create Buckets")
    log_output = gr.components.Textbox(label="Log")
    bucket_button.click(fn=make_config,
