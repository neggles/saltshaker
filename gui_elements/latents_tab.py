...
def make_latents(model_path, config_path, buckets_path, batch, progress=gr.Progress()):
    try:
...
def load():
    ...
    pickle_save_path = gr.Textbox(lines=1, label="Buckets Save Path", value="buckets.pkl", interactive=True)
    ...
    latents_button.click(fn=make_latents,
                         inputs=[model_path, config_load_path, pickle_save_path, train_batch],
                         outputs=[log_output])
